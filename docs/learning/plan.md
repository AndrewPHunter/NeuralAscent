...
| **Training samples, it does not minimise** 🆕 | SGD with noise has a stationary distribution `p(w) ∝ exp(−L(w)/T)` over parameters, with `T` set by learning rate and batch size — a Gibbs measure on parameter space. Deep networks converge to **limit cycles**, not minima (arXiv:1710.11029). Same construction as softmax, different space: softmax is Gibbs over the vocabulary at temperature `√d`; SGD is Gibbs over the parameters at temperature `lr/batch`. See [§8](./geometric-lens.md#8-training-samples-a-distribution-it-does-not-find-a-minimum). |
...
| **The sum is the path integral** 🆕 | `Σⱼ exp(zⱼ)` is a partition function on a finite index set — a **discrete sum over histories**, in Feynman's sense. `Z = Σ exp(−E/T)` and `Z = ∫D[x] exp(−S/ħ)` are one construction differing only in whether the index set is discrete. The sum is not an approximation to the integral. This is the bridge to module 6's continuous formulations. See [§7](./geometric-lens.md#7-the-sum-is-the-path-integral). |
